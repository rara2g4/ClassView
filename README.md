...
現在の`course-works.json`には表示確認用の架空制作物が登録されています。実在する学生・団体の作品ではありません。画像のみ、外部リンクのみ、画像と外部リンクの3パターンを、Web、ゲーム、ネットワーク、データベース、デザイン、AI、グループ制作の授業で確認できます。リンク先の`samples/works/`も架空作品の説明用に用意した静的ページです。実運用へ移行する際は、掲載許可を確認した実際の制作物へ管理ツールから差し替えてください。
